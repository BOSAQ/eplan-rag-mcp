...
- **`references/pitfalls.md`** — CRITICAL: the command-blocking issue (message loop / monitor thread), `using`/`Dispose` discipline, sequential execution model, error-handling rules, and the C# 5-only script engine whose compile errors masquerade as timeouts (#10).
...
10. **Write scripts in C# 5 — no `?.`, no `$"..."`, no `{ ["k"] = v }`, no `nameof`.** EPLAN's script engine predates C# 6, and a compile error is *silent*: `ExecuteScript` returns success, the script never runs, and a caller waiting on its output just sees a timeout — so the bug gets misread as a hung or blocked EPLAN. **On any script timeout, read EPLAN's system-message tree for the `CS####` line before suspecting anything else.** See `references/pitfalls.md` #10.
